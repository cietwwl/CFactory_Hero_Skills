  ...
  extends ActiveSkill
{
  ...
  public SkillResult use(Hero hero, String[] args)
  {
    ...
    Messaging.send(player, "$1 is a level $2 $3 with $4 health!", tHero.getPlayer().getDisplayName(), Integer.valueOf(tHero.getTieredLevel(tHero.getHeroClass())), tHero.getHeroClass().getName(), tHero.getPlayer().getHealth());
    Messaging.send(player, "$5 pos: $1: $2,$3,$4", location.getWorld().getName(), Integer.valueOf(x), Integer.valueOf(y), Integer.valueOf(z), tHero.getPlayer().getDisplayName());
    Location hLoc = hero.getPlayer().getLocation();
    int hX = hLoc.getBlockX();
    int hY = hLoc.getBlockY();
    int hZ = hLoc.getBlockZ();
    
    Messaging.send(player, "Your current pos: $1: $2, $3, $4", hLoc.getWorld().getName(), Integer.valueOf(hX), Integer.valueOf(hY), Integer.valueOf(hZ));
